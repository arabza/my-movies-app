...
const RecommendedMovies: React.FC<IRecommendedMoviesProps> = ({ movieId, type }) => {
    ...
    if (loading) {
        return React.createElement("div", { className: "mt-10 p-8" }, [
            React.createElement("h2", { className: "text-2xl font-bold mb-4", key: "title" }, 
                    ...
                    recommendedMovies.map(movie => 
                        React.createElement("div", { key: movie.id, className: "w-[200px] flex-shrink-0" },
                            React.createElement(Link, { 
                                href: { pathname: `/${type ?? 'movie'}/${movie.id}` },
